...
int web_sql_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(map_handle, map_server_id, map_server_pw, map_server_ip, map_server_port, map_server_db)) {
		ShowError("Couldn't connect with uname='%s',passwd='%s',host='%s',port='%d',database='%s'\n",
	...
	if (strlen(default_codepage) > 0) {
		if (SQL_ERROR == Sql_SetEncoding(map_handle, default_codepage))
			Sql_ShowDebug(map_handle);
	}
...
int do_init(int argc, char** argv) {
	...
	http_server->Post("/api/v1/register", register_post);
