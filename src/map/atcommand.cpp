...
ACMD_FUNC(mapmove)
{
	...
	if (map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(fd, msg_txt(sd,248)); // You are not authorized to warp from your current map.
		return -1;
	}
...
ACMD_FUNC(reload) {
	...
	} else if (strstr(command, "whisper") || strncmp(message, "whisper", 3) == 0) {
		whisper_reload();
		clif_displaymessage(fd, "Whisper Custom DB has been reloaded.");
	} else if (strstr(command, "refine") || strncmp(message, "refine", 4) == 0) {
		refinedb_reload();
		clif_displaymessage(fd, "All refine DB related has been reloaded.");
	}
...
ACMD_FUNC(autotrade) {
	...
	// @at tax
	std::shared_ptr<s_autoatrade_tax> AutotradeTax = autotrade_tax_db.find(sd->bl.m);
	if (AutotradeTax != nullptr) {
		if(sd->status.zeny < AutotradeTax->tax){
			char msg[CHAT_SIZE_MAX];
			sprintf(msg, "คุณมี Zeny ไม่พอสำหรับเสียภาษีการใช้งาน @at,ต้องการ %d Zeny",AutotradeTax->tax);
			clif_displaymessage(fd, msg);
			return -1;
		}
	}	
	
	pc_payzeny(sd, AutotradeTax->tax, LOG_TYPE_VENDING, NULL);
...
#include "../custom/atcommand_def.inc"
		...
		ACMD_DEF2("reloadwhisper",reload),
		ACMD_DEF2("reloadrefine",reload),
