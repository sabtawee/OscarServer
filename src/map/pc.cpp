...
void pc_makesavestatus(struct map_session_data *sd) {
	...
	if(map_getmapflag(sd->bl.m, MF_NOSAVE) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP )) {
...
enum e_additem_result pc_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type) {
	...
	if (id->type == IT_RELIC) status_calc_pc(sd, SCO_NONE);
...
char pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	if (mem == IT_RELIC) status_calc_pc(sd, SCO_NONE);
	return 0;
}
...
bool pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	switch( nameid ) {
		...
		case ITEMID_N_FLY_WING:
			if( mapdata->flag[MF_NOTELEPORT] || mapdata_flag_gvg2(mapdata) || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) ) {
		...
		case ITEMID_SIEGE_TELEPORT_SCROLL:
			...
			if( mapdata->flag[MF_NORETURN] && nameid != ITEMID_WING_OF_FLY && nameid != ITEMID_GIANT_FLY_WING && nameid != ITEMID_N_FLY_WING || map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) )
				return false;
			break;
...
/**
 * reduce baseexp got from killing mob and split into exp book
 * @param sd Player
 * @param baseexp EXP from mob
 * @return
 **/
static t_exp pc_baseexp_book(struct map_session_data *sd,t_exp baseexp)
{
	nullpo_ret(sd);
	
	ShowDebug("Exp Book [BASE => %d]\n",baseexp);
	
	int64 exp_percent = pc_readregistry(sd, reference_uid(add_str("baseexp_book_split"), 0));
	int64 current_baseexp = pc_readregistry(sd, reference_uid(add_str("baseexp_book"), 0));
	int64 exp_limit = static_cast<int>(mapreg_readreg(add_str("$@exp_limit")));
	int64 show_baseexp_book;

	if(current_baseexp != exp_limit){
		if (exp_percent < 100){
			baseexp = baseexp * (100-exp_percent) / 100;
			show_baseexp_book = baseexp * (100-exp_percent) / 100;
		}else
			show_baseexp_book = baseexp;

		pc_setregistry(sd, reference_uid(add_str("baseexp_book"), 0), current_baseexp + baseexp);
		
		if(baseexp > 0){
			char msg[255];
			sprintf(msg,"[Exp Book] +%I64d Base Experience points",baseexp);
			clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], msg, false, SELF);			
		}
		
		int64 after_exp = pc_readregistry(sd, reference_uid(add_str("baseexp_book"), 0));
		if(after_exp > exp_limit){
			pc_setregistry(sd, reference_uid(add_str("baseexp_book"), 0), exp_limit);
		}
	}
...
/**
 * reduce jobexp got from killing mob and split into exp book
 * @param sd Player
 * @param jobexp EXP from mob
 * @return
 **/
static t_exp pc_jobexp_book(struct map_session_data *sd,t_exp jobexp)
{
	nullpo_ret(sd);
	
	ShowDebug("Exp Book [JOB => %d]\n",jobexp);

	int64 job_percent = pc_readregistry(sd, reference_uid(add_str("jobexp_book_split"), 0));
	int64 current_jobexp = pc_readregistry(sd, reference_uid(add_str("jobexp_book"), 0));
	int64 exp_limit = static_cast<int>(mapreg_readreg(add_str("$@exp_limit")));
	int64 show_baseexp_book;

	if(current_jobexp != exp_limit){
		if (job_percent < 100){
			jobexp = jobexp * (100-job_percent) / 100;
			show_baseexp_book = jobexp * (100-job_percent) / 100;
		}else
			show_baseexp_book = jobexp;

		pc_setregistry(sd, reference_uid(add_str("jobexp_book"), 0), current_jobexp + jobexp);
		
		if(jobexp > 0){
			char msg[255];
			sprintf(msg,"[Exp Book] +%I64d Job Experience points",jobexp);
			clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], msg, false, SELF);			
		}
		
		int64 after_exp = pc_readregistry(sd, reference_uid(add_str("jobexp_book"), 0));
		if(after_exp > exp_limit){
			pc_setregistry(sd, reference_uid(add_str("jobexp_book"), 0), exp_limit);
		}
	}
...
void pc_gainexp(struct map_session_data *sd, struct block_list *src, t_exp base_exp, t_exp job_exp, uint8 exp_flag)
{
	...
	if(sd->bl.prev == NULL || pc_isdead(sd) || sd->sc.data[SC_TRICKDEAD] || sd->state.vending)
		return;
	...
	if(battle_config.feature_exp_book == 1){
		
		ShowDebug("Exp Book [TRUE]\n");
		
		if(base_exp)
			base_exp -= pc_baseexp_book(sd,base_exp);
			
		if(job_exp)
			job_exp -= pc_jobexp_book(sd,job_exp);
	}else{
		ShowDebug("Exp Book [FALSE]\n");
	}
...
void pc_deathmatch(struct map_session_data *sd, clr_type clrtype)
{
	...
	if (map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && battle_config.feature_cellpvp)
	{
		do
		{
		...
		pc_setstand(sd, true);
		pc_setrestartvalue(sd, 3);
		status_percent_heal(&sd->bl, battle_config.cellpvp_hp_rate, battle_config.cellpvp_sp_rate);
		...
		if (sd && battle_config.cellpvp_autorebuff)
		{
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if(battle_config.bone_drop==2
		|| (battle_config.bone_drop==1 && mapdata->flag[MF_PVP]|| map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && sd->state.cellpvp))
	{
	...
	// Addon Cell PVP [Napster]
	if (map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP ) && battle_config.feature_cellpvp && sd->state.cellpvp)
	{
		add_timer(tick+battle_config.cellpvp_repawn_delay, pc_deathmatch_timer, sd->bl.id, 0);
		return 1|8;
	}	
