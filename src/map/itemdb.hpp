...
//		some script commands should be revised as well...
	...
	struct script_code *iventory_script;//Script
	struct script_code *storage_script;//Script
	struct {
	...
	~item_data() {
		...
		if (this->iventory_script){
			script_free_code(this->iventory_script);
			this->iventory_script = nullptr;
		}
		
		if (this->storage_script){
			script_free_code(this->storage_script);
			this->storage_script = nullptr;
		}
...
struct s_refine_target {
	uint16 RandomoptGroup;
	std::vector<t_itemid> ItemNameid;
};

struct s_refine_randomopt {
	uint16 Refine;
	std::unordered_map<item_types, std::shared_ptr<s_refine_target>> EquipmentType;
};

class RefineRandomOptDatabase : public TypesafeYamlDatabase<uint16, s_refine_randomopt> {
public:
