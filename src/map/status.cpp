...
int status_calc_pc_sub(struct map_session_data* sd, uint8 opt)
{
	...
	if (battle_config.show_rank > 0) {
		uint16 title_id = pc_readregistry(sd, reference_uid(add_str("title_id"), 0));
		// RANK_BONUS
		if(title_id){
			std::shared_ptr<s_rank_title> rank_title = rank_title_db.find(title_id);
			if (rank_title != nullptr) {
...
// ----- Costume Collection -----
	struct npc_data *collection = npc_name2id("COLLECTION_BONUS_CTRL");
	if (collection && collection->subtype == NPCTYPE_SCRIPT){
		run_script(collection->u.scr.script, 0, sd->bl.id, collection->bl.id);
	}
...
// ----- RELIC STORAGE -----
	for (i = 0; i < MAX_STORAGE; i++){
		...
		if (get_item_type(sd->storage.u.items_storage[i].nameid) != IT_RELIC)
			continue;
			
		struct item_data *storage_relic = itemdb_search(sd->storage.u.items_storage[i].nameid);
		if (storage_relic->storage_script){
			run_script(storage_relic->storage_script, 0, sd->bl.id, 0);
			if (!calculating)
				return 1;
		}
	}
...
// ----- RELIC INVENTORY -----
	for (i = 0; i < MAX_INVENTORY; i++){
		...
		if (get_item_type(sd->inventory.u.items_inventory[i].nameid) != IT_RELIC)
			continue;
			
		struct item_data *inventory_relic = itemdb_search(sd->inventory.u.items_inventory[i].nameid);
		if (inventory_relic->iventory_script){
			run_script(inventory_relic->iventory_script, 0, sd->bl.id, 0);
			if (!calculating)
				return 1;
		}
	}
