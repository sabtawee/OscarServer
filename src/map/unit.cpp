...
int unit_walktoxy( struct block_list *bl, short x, short y, unsigned char flag)
{
	...
	if (bl->type == BL_PC)
	{
		...
		if (sd && sd->cellpvp_walkout_tick && !map_getcell( sd->bl.m, x, y, CELL_CHKPVP ) )
		{
			if (DIFF_TICK(tick, sd->cellpvp_walkout_tick) < battle_config.cellpvp_exit_delay)
			{
				e_tick = (battle_config.cellpvp_exit_delay - DIFF_TICK(tick, sd->cellpvp_walkout_tick)) / 1000;
				char e_msg[150];
				if (e_tick > 99)
...
int unit_skilluse_id(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv)
{
	...
	if( bl && src->type == BL_PC && bl->type == BL_PC && src->id != target_id)
	{
		struct map_session_data *dstsd = (TBL_PC*)bl;
		 ...
		 if( dstsd && dstsd->state.cellpvp != ((TBL_PC*)src)->state.cellpvp )
			 return 0;
	}
	return unit_skilluse_id2(
