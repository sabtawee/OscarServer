...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC: {
			...
			if( sd->state.cellpvp && s_bl->type == BL_PC && ((TBL_PC*)s_bl)->state.cellpvp )
				state |= BCT_ENEMY; 
			if( sd->state.killable || sd->state.vending ) {
	...
	{ //Non pvp/gvg, check party/guild settings.
		...
		{
			int s_party = status_get_party_id(s_bl);
			if(s_party && s_party == status_get_party_id(t_bl) && !(battle_config.cellpvp_party_attackable && map_getcell( t_bl->m, t_bl->x, t_bl->y, CELL_CHKPVP )) )		// Addon Cell PVP [Napster]
				state |= BCT_PARTY;
		}
...
// rAthena
	...
	// ExpBook
	{ "feature.exp_book",					&battle_config.feature_exp_book,			 	1,		0,      1,				},
