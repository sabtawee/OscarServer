...
int map_pvp_area(struct map_session_data* sd, bool flag)
{
	...
	{
		case 1:
			char walkin_msg[CHAT_SIZE_MAX];
			sprintf(walkin_msg, "[%s] ได้เดินเข้าสู่ \"ซัดหน้าโซน\", ใครหิวก็เข้ามา", sd->status.name);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], walkin_msg, false, ALL_CLIENT);		
			clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE,ALL_SAMEMAP);
			//clif_map_property_pvp(&sd->bl,ALL_SAMEMAP);
			if (sd->pvp_timer == INVALID_TIMER) {
				...
				sd->pvp_won   = 0;
				sd->pvp_lost  = 0;
				sd->state.cellpvp  = true;
				sd->cellpvp_walkout_tick = gettick();
			}
			break;
