...
static inline int itemtype(t_itemid nameid) {
	...
	return (type == IT_PETEGG) ? IT_ARMOR : (type == IT_RELIC) ? IT_ARMOR : type;
}
...
void clif_dropflooritem( struct flooritem_data* fitem, bool canShowEffect ){
	...
	if( canShowEffect ){
		...
		// DropEffect
		std::shared_ptr<s_drop_effect> DropEffect = drop_effect_db.find((item_types)itemtype(fitem->item.nameid));
		if (DropEffect != nullptr) {
			p.showdropeffect = 1;
 			p.dropeffectmode = DropEffect->DropEffect - 1;
		}else{
			p.showdropeffect = 0;
...
void clif_buylist( struct map_session_data *sd, struct npc_data *nd ){
	...
	if(sd->state.costume_open == true){	
		...
		sd->cos_sys.count = (int)SqlStmt_NumRows(stmt);
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP]?1:0 || (bl->type == BL_PC && ((TBL_PC *)bl)->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
		...
		((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP]?1:0)<<5)| // COUNT_PK - Show the PvP counter
...
void donate_level_buff(struct map_session_data *sd){
	nullpo_retv(sd);
...
/*==========================================
 * function that apply on player when loading into map
 *------------------------------------------*/
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if( !sd->state.cellpvp && map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) )
		map_pvp_area(sd, 1);
	else if( sd->state.cellpvp && !map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) )
		map_pvp_area(sd, 0);	
...
void clif_parse_captcha_register(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160316
	if (!pc_has_permission(sd, PC_PERM_USE_MACRO_INTERFACE)) {
		clif_displaymessage(sd->fd, "You are not autorized to use the macro interface");
		return;
	}

	const struct PACKET_CZ_CAPTCHA_REGISTER *p = (struct PACKET_CZ_CAPTCHA_REGISTER *)RFIFOP( fd, 0 );
	macro_captcha_register(sd, p->imageSize, p->answer);
#endif
}

void clif_captcha_upload_request(struct map_session_data *sd, const char *captcha_key, const int captcha_flag) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);
	...
	struct PACKET_ZC_CAPTCHA_UPLOAD_REQUEST p = { };
...
void clif_parse_captcha_upload(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160316
	if (!pc_has_permission(sd, PC_PERM_USE_MACRO_INTERFACE)) {
		clif_displaymessage(sd->fd, "You are not authorized to use the macro interface");
		return;
	}

	const struct PACKET_CZ_CAPTCHA_UPLOAD_REQUEST_ACK *p = (struct PACKET_CZ_CAPTCHA_UPLOAD_REQUEST_ACK *)RFIFOP( fd, 0 );

	const int upload_size = p->PacketLength - sizeof(struct PACKET_CZ_CAPTCHA_UPLOAD_REQUEST_ACK);
	macro_captcha_register_upload(sd, p->captchaKey, upload_size, p->imageData);
#endif
}

void clif_captcha_upload_end(struct map_session_data *sd) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);

	int fd = sd->fd;

	struct PACKET_ZC_CAPTCHA_UPLOAD_REQUEST_STATUS p = { };
...
void clif_parse_captcha_preview_request(int fd, struct map_session_data *sd) {
	...
	const struct PACKET_CZ_CAPTCHA_PREVIEW_REQUEST *p = (struct PACKET_CZ_CAPTCHA_PREVIEW_REQUEST *)RFIFOP( fd, 0 );
	macro_captcha_preview(sd, p->captchaID);
#endif
}

void clif_captcha_preview_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size, const int captcha_flag) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);
	nullpo_retv(captcha_key);
	...
	struct PACKET_ZC_CAPTCHA_PREVIEW_REQUEST p = { };
...
void clif_captcha_preview_request_download(struct map_session_data *sd, const char *captcha_key, const int chunk_size, const char *chunk_data) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);
	nullpo_retv(captcha_key);
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_CAPTCHA_PREVIEW_REQUEST_DOWNLOAD *p = (struct PACKET_ZC_CAPTCHA_PREVIEW_REQUEST_DOWNLOAD *)WFIFOP( fd, 0 );
...
void clif_macro_detector_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);
	nullpo_retv(captcha_key);
	...
	struct PACKET_ZC_MACRO_DETECTOR_REQUEST p = { };
...
void clif_macro_detector_request_download(struct map_session_data *sd, const char *captcha_key, const int chunk_size, const char *chunk_data) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);
	nullpo_retv(captcha_key);
	nullpo_retv(chunk_data);
	...
	struct PACKET_ZC_MACRO_DETECTOR_REQUEST_DOWNLOAD *p = (struct PACKET_ZC_MACRO_DETECTOR_REQUEST_DOWNLOAD *)WFIFOP( fd, 0 );
...
void clif_macro_detector_request_show(struct map_session_data *sd) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);

	int fd = sd->fd;

	struct PACKET_ZC_MACRO_DETECTOR_SHOW p = { };
	p.PacketType = HEADER_ZC_MACRO_DETECTOR_SHOW;
	p.retryCount = sd->macro_detect.retry;
	p.timeout = battle_config.macro_detect_timeout;

	clif_send( &p, sizeof( p ), &sd->bl, SELF );
#endif
}

void clif_parse_macro_detector_download_ack(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160316
	if (sd->macro_detect.retry != 0) {
		// const struct PACKET_CZ_MACRO_DETECTOR_DOWNLOAD *p = (struct PACKET_CZ_MACRO_DETECTOR_DOWNLOAD *)RFIFOP( fd, 0 );
		clif_macro_detector_request_show(sd);
	}
#endif
}

void clif_parse_macro_detector_answer(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160316
	if (sd->macro_detect.retry != 0) {
		const struct PACKET_CZ_MACRO_DETECTOR_ANSWER *p = (struct PACKET_CZ_MACRO_DETECTOR_ANSWER *)RFIFOP( fd, 0 );
		macro_detector_process_answer(sd, p->answer);
	}
#endif
}

void clif_macro_detector_status(struct map_session_data *sd, enum macro_detect_status stype) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);

	int fd = sd->fd;

	struct PACKET_ZC_MACRO_DETECTOR_STATUS p = { };
...
void clif_parse_macro_reporter_select(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160330
	if (!pc_has_permission(sd, PC_PERM_USE_MACRO_INTERFACE)) {
		clif_displaymessage(sd->fd, "You are not autorized to use the macro interface");
		return;
	...
	const struct PACKET_CZ_MACRO_REPORTER_SELECT *p = (struct PACKET_CZ_MACRO_REPORTER_SELECT *)RFIFOP( fd, 0 );
	macro_reporter_area_select(sd, p->xPos, p->yPos, p->RadiusRange);
#endif
}

void clif_macro_reporter_select(struct map_session_data *sd, std::vector<int> aid_list) {
#if PACKETVER >= 20160330
	...
	const int len = sizeof(struct PACKET_ZC_MACRO_REPORTER_SELECT) + sizeof(int) * aid_list.size();

	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MACRO_REPORTER_SELECT *p = (struct PACKET_ZC_MACRO_REPORTER_SELECT *)WFIFOP(fd, 0);
	p->PacketType = HEADER_ZC_MACRO_REPORTER_SELECT;
	p->PacketLength = len;
	for (int i = 0; i < aid_list.size(); i++)
		p->AID[i] = aid_list.at(i);
	WFIFOSET(fd, len);
#endif
}

void clif_parse_macro_reporter_ack(int fd, struct map_session_data *sd) {
#if PACKETVER >= 20160316
	if (!pc_has_permission(sd, PC_PERM_USE_MACRO_INTERFACE)) {
		clif_displaymessage(sd->fd, "You are not autorized to use the macro interface");
		return;
	...
	const struct PACKET_CZ_MACRO_REPORTER_ACK *p = (struct PACKET_CZ_MACRO_REPORTER_ACK *)RFIFOP( fd, 0 );
	struct map_session_data *tsd = map_id2sd(p->AID);
	if (tsd == NULL) {
		char buf[256];
		sprintf(buf, "Player %d is unavailable.", p->AID);
	...
	if (captcha_db.size() == 0) {
		clif_macro_reporter_status(sd, MCR_NO_DATA);
		return;
	}
	macro_reporter_process(sd, tsd);
	clif_macro_reporter_status(sd, MCR_MONITORING);
#endif
}

void clif_macro_reporter_status(struct map_session_data *sd, enum macro_report_status stype) {
#if PACKETVER >= 20160330
	nullpo_retv(sd);

	int fd = sd->fd;

	struct PACKET_ZC_MACRO_REPORTER_STATUS p = { };
...
void clif_parse_open_ui( int fd, struct map_session_data* sd ){
	switch( RFIFOB(fd,2) ){
		case IN_MACRO_REGISTER_UI:
			clif_ui_open( *sd, OUT_CAPTCHA_UI, 0 );
			break;
		case IN_MACRO_DETECTOR_UI:
			clif_ui_open( *sd, OUT_MACRO_UI, 0 );
			break;
...
void clif_parse_refineui_refine( int fd, struct map_session_data* sd ){
	...
	if( cost->chance >= ( rnd() % 10000 ) ){
		...
		// RefineRandomOpt
		std::shared_ptr<s_refine_randomopt> RefineRandomOpt = refine_randomopt_db.find(item->refine);
		if (RefineRandomOpt != nullptr) {
			std::shared_ptr<s_refine_target> EqType = util::umap_find(RefineRandomOpt->EquipmentType, get_item_type(item->nameid));
			if(EqType != nullptr){
				if(util::vector_exists( EqType->ItemNameid, item->nameid )){
					std::shared_ptr<s_random_opt_group> group = random_option_group.find(EqType->RandomoptGroup);
					if (group != nullptr) {
						group->apply( *item );
		...
		// RefineAnnounce
		if(battle_config.refine_success_annouce > 0 && item->refine >= battle_config.refine_success_annouce){
			char output_msg[CHAT_SIZE_MAX];
			memset(output_msg, '\0', sizeof(output_msg));
			std::shared_ptr<item_data> item_data = item_db.find(item->nameid);
			sprintf(output_msg, "[%s] ได้ตีบวกไอเท็ม <%s> ขึ้น +%d สำเร็จ",sd->status.name,item_data->ename.c_str(),item->refine);
			clif_broadcast(NULL, output_msg, strlen(output_msg) + 1, BC_BLUE, ALL_CLIENT);
		}
		...
		// RefineAnnounce
		if(battle_config.refine_fail_annouce > 0 && (item->refine+1) >= battle_config.refine_fail_annouce){
			char output_msg[CHAT_SIZE_MAX];
			memset(output_msg, '\0', sizeof(output_msg));
			std::shared_ptr<item_data> item_data = item_db.find(item->nameid);
			sprintf(output_msg, "[%s] ได้ตีบวกไอเท็ม <%s> ขึ้น +%d แหก",sd->status.name,item_data->ename.c_str(),item->refine+1);
			clif_broadcast(NULL, output_msg, strlen(output_msg) + 1, 0xff0000, ALL_CLIENT);
		}	
