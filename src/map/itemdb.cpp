...
uint64 ItemDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "IventoryScript")) {
		std::string iventory_script;

		if (!this->asString(node, "IventoryScript", iventory_script))
			return 0;

		if (exists && item->iventory_script) {
			script_free_code(item->iventory_script);
			item->iventory_script = nullptr;
		}

		item->iventory_script = parse_script(iventory_script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["IventoryScript"]), SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	} else {
		if (!exists) 
			item->iventory_script = nullptr;
	}
	
	if (this->nodeExists(node, "StorageScript")) {
		std::string storage_script;

		if (!this->asString(node, "StorageScript", storage_script))
			return 0;

		if (exists && item->storage_script) {
			script_free_code(item->storage_script);
			item->storage_script = nullptr;
		}

		item->storage_script = parse_script(storage_script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["StorageScript"]), SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	} else {
		if (!exists) 
			item->storage_script = nullptr;
	}		
...
uint64 RefineRandomOptDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (!this->nodesExist(node, { "Refine", "EquipmentType" }))
		return 0;	
	
		uint16 Refine;
		
	if (!this->asUInt16(node, "Refine", Refine))
		return 0;		
	
	std::shared_ptr<s_refine_randomopt> RefineRandomOpt = this->find(Refine);
	...
	if (!exists) {
		...
		RefineRandomOpt = std::make_shared<s_refine_randomopt>();
		RefineRandomOpt->Refine = Refine;
	}	
	
	if( this->nodeExists( node, "EquipmentType" ) ){
		for( const auto& EqType : node["EquipmentType"] ){
			
			std::string type_name;
			c4::from_chars(EqType.key(), &type_name);

			std::string mode_constant = "IT_" + type_name;
			int64 constant;

			if (!script_get_constant(mode_constant.c_str(), &constant) || constant < IT_HEALING || constant > IT_MAX) {
				this->invalidWarning(EqType, "[RefineRandomOpt] : Refine +%d, Invalid item type %s, skipping.\n",RefineRandomOpt->Refine, type_name.c_str());
				continue;
			}
			
			item_types it_type = static_cast<item_types>(constant);
			
			std::shared_ptr<s_refine_target> target = util::umap_find(RefineRandomOpt->EquipmentType, it_type);
			bool type_exists = target != nullptr;

			if (!type_exists)
				target = std::make_shared<s_refine_target>();
				
			if (this->nodeExists(EqType, "RandomOptGroup")) {
				uint16 randomoptgroup;

				if (!this->asUInt16(EqType, "RandomOptGroup", randomoptgroup))
					return 0;

				std::shared_ptr<s_random_opt_data> RandomoptGroup = random_option_db.find(randomoptgroup);
				
				if(RandomoptGroup == nullptr){
					this->invalidWarning(EqType["RandomOptGroup"], "[RefineRandomOpt] : Refine +%d, Item Type : %s, RandomOptGroup %d is does not exist.\n", RefineRandomOpt->Refine, type_name.c_str(),randomoptgroup);
					return 0;
				}				
				target->RandomoptGroup = randomoptgroup;
			}else{
				if( !exists ){
					this->invalidWarning(node["EquipmentType"], "[RefineRandomOpt] : Refine +%d, Item Type : %s, is skipped because no \"RandomOptGroup\" node.\n", RefineRandomOpt->Refine, type_name.c_str());
					continue;
				}
			}
				
			if (this->nodeExists(EqType, "TargetItems"))
			{
				for (const auto& TargetItemsNode : EqType["TargetItems"])
				{
					if (!this->nodeExists(TargetItemsNode, "Item"))
					{
						return 0;
					}
					...
					if (!this->asString(TargetItemsNode, "Item", ItemAegisName))
					{
						return 0;
					}
					...
					if(items == nullptr){
						this->invalidWarning(TargetItemsNode["Item"], "[RefineRandomOpt] : Refine +%d, Item Type : %s, Item \"%s\" is does not exist.\n", RefineRandomOpt->Refine, type_name.c_str(),ItemAegisName.c_str());
						continue;
					}
					
					if(items->type != it_type){
						this->invalidWarning(TargetItemsNode["Item"], "[RefineRandomOpt] : Refine +%d, Item Type : %s, item \"%s\" type is invalid.\n", RefineRandomOpt->Refine, type_name.c_str(),ItemAegisName.c_str());
						continue;
					}

					target->ItemNameid.push_back(items->nameid);
				}
				
				if (!type_exists)
					RefineRandomOpt->EquipmentType.insert({it_type , target});
			}				
		}
	}
...
/*==========================================
 * look through item db and return item_type type
 * @param nameid
 *------------------------------------------*/
item_types get_item_type(t_itemid nameid) {
	std::shared_ptr<item_data> id;
